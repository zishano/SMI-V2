...
from utils import *
from utils.build_model_tome import build_model_tome
...
def get_args_parser():
    ...
    parser.add_argument("--calib-batchsize", default=8,
                        type=int, help="batchsize of calibration set")
    parser.add_argument("--val-batchsize", default=50,
    ...
    parser.add_argument('--ldr', default=0, type=float)
...
def get_teacher(name):
    teacher_name = {'deit_tiny_16_imagenet': 'deit_tiny_patch16_224',
                    'deit_small_16_imagenet': 'deit_small_patch16_224',
                    'deit_base_16_imagenet': 'deit_base_patch16_224',

                    'vit_base_16_imagenet': 'vit_base_patch16_224',
                    'vit_small_16_imagenet': 'vit_small_patch16_224',
                    'vit_tiny_16_imagenet': 'vit_tiny_patch16_224',

                    'swin_tiny_16_imagenet': 'swin_tiny_patch4_window7_224',
                    'swin_small': 'swin_small_patch4_window7_224',
                    }
    if args.model.split("_")[-1]=='imagenet':
        # teacher=build_model_tome(teacher_name[name], Pretrained=True)
        teacher=build_model_tome(teacher_name[name], Pretrained=True)
    else:
...
def main():
    ...
    # Build model
    # print("aefughoeaiu")
    model = get_student(args.model)(pretrained=True, cfg=cfg)##学生模型，只在最后校正一次
    model = model.to(device)
    model.eval()
    print(count_parameters(model),"model")
    # print("aseugfo9b")
    teacher=get_teacher(args.model)##教师模型，构建的时候重写forward,用于获得attention_map相关信息,通过校正能生成更好的校正图片
    # register_custom_flop_counters(teacher)
    teacher=teacher.to(device)
    teacher.eval()
    # ##使用tome对模型进行处理
    # tome.patch.timm(teacher)
    # teacher.r = 64
    # ##使用tome对模型进行处理
    print(count_parameters(model),"teacher")
    ...
    if args.mode == 0:
        ...
        img_tag = "{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-quantization{}{}-smi".format(args.ldr, iterations, lr_g, adv, bn, oh, tv1, tv2, l2, str(prune_it), str(prune_ratio),args.w_bit,args.a_bit)
        ...
        synthesizer = SMI(
            ...
            synthesis_batch_size=128, sample_batch_size=args.calib_batchsize,
            adv=adv, bn=bn, oh=oh, tv1=tv1,tv2=tv2, l2=l2, ldr=args.ldr,ratio=args.ratio,
        ...
        print(count_parameters(model),args.model)
